...
static inline quat quat_from_euler_xyz(float x, float y, float z)
{
    return quat_mul(
        quat_from_angle_axis(z, vec3(0,0,1)),
        quat_mul(
            quat_from_angle_axis(y, vec3(0,1,0)),
            quat_from_angle_axis(x, vec3(1,0,0))));
}

static inline void quat_swing_twist(
    quat& swing,
    quat& twist,
    const quat q,
    const vec3 axis)
...
static inline float quat_frobenius_distance(quat q, quat p)
{
